...
const CFG = {
  ...
  ABA_LOG: 'Log',
  ABA_LEADS: 'Leads',
  ABA_PROCEDIMENTOS: 'ProcedimentosSite',
  ...
  PROCEDIMENTOS: [
    ...
    {
      key: 'browlamination',
      sheet: 'Brow Lamination',
      nome: 'Brow Lamination',
      precoPromocional: '',
      observacaoPreco: 'sobrancelhas alinhadas e modeladas',
      tipoPreco: 'A partir de',
      descricao: 'Técnica para alinhar, modelar e valorizar os fios naturais das sobrancelhas.'
    }
  ]
};

const PUBLIC_CACHE_KEY = 'STUDIO_PUBLIC_V30';
...
function dadosPublicos_(){
  ...
  const ss = planilha_();
  const precisaPreparar = !ss.getSheetByName(CFG.ABA_PROCEDIMENTOS) || CFG.PROCEDIMENTOS.some(cfg => !ss.getSheetByName(cfg.sheet));
  if(precisaPreparar) garantirEstruturaPadroes_(ss);
  const procedures = {};
  procedimentosConfig_(ss).forEach(cfg => {
    try{ procedures[cfg.key] = procedimentoPublico_(lerProcedimento_(cfg.key, ss)); }catch(_){ }
  });
  const payload = {
    success:true,
    ok:true,
...
function INSTALAR(){
  ...
  const ss = planilha_();
  sincronizarCadastroProcedimentos_(ss);
...
function configurarAbaProcedimento_(config){
  ...
  const rows = [
    ...
    ['Preço promocional', precoPromo, 'Valor principal. Ex.: 500'],
    ['Desconto calculado', '', 'Automático'],
    ['Tipo do preço', valorOuPadrao_(atual,['tipo do preço','tipo do preco'],config.tipoPreco || 'A partir de'), 'Escolha: A partir de ou Por'],
  ...
  sh.getRange(1,1,Math.max(19,sh.getLastRow()),3).clearContent().clearFormat();
  ...
  sh.setRowHeights(1,rows.length,30);
  sh.getRange('A1:C1').setBackground('#17130F').setFontColor('#FFFFFF').setFontWeight('bold').setHorizontalAlignment('center');
  sh.getRange(`A2:A${rows.length}`).setBackground('#F5EFE8').setFontWeight('bold');
  sh.getRange(`B2:B${rows.length}`).setBackground('#FFF8E8');
  sh.getRange('B5').setBackground('#E9F6E8').setFontWeight('bold').setFontColor('#27632A');
  sh.getRange(`A1:C${rows.length}`).setVerticalAlignment('middle').setWrap(true)
...
function doPost(e){
  try{
    ...
    const mapa = {
      ...
      criarProcedimento: () => criarProcedimento_(u,d),
...
function listarProcedimentos_(u){
  const ss = planilha_();
  garantirEstruturaPadroes_(ss);
  const procedures = {};
  procedimentosConfig_(ss).forEach(cfg => { try{ procedures[cfg.key] = lerProcedimento_(cfg.key, ss); }catch(_){ } });
  return {ok:true,procedures:procedures,usuario:publicUser_(u)};
}

function criarProcedimento_(u,d){
  const nome = clean_(d.nome,120);
  const descricao = clean_(d.descricao,900);
  if(nome.length < 2) throw new Error('Informe o nome do procedimento.');
  const ss = planilha_();
  const configs = procedimentosConfig_(ss);
  let key = slug_(nome);
  if(!key) key = 'procedimento';
  let base = key, n = 2;
  while(configs.some(x => x.key === key)) key = base + n++;
  let sheet = nome.replace(/[\\/\?\*\[\]:]/g,' ').replace(/\s+/g,' ').trim().slice(0,90) || 'Procedimento';
  let sheetBase = sheet, sn = 2;
  while(ss.getSheetByName(sheet)) sheet = (sheetBase.slice(0,85) + ' ' + sn++).slice(0,90);
  const cfg = {key:key,sheet:sheet,nome:nome,precoPromocional:'',observacaoPreco:'',tipoPreco:'A partir de',descricao:descricao};
  const cadastro = garantirCadastroProcedimentos_(ss);
  cadastro.appendRow([key,sheet,nome,configs.length+1]);
  configurarAbaProcedimento_(cfg);
  limparCachePublico_();
  log_('CRIAR_PROCEDIMENTO', nome, u);
  return {ok:true,procedure:lerProcedimento_(key)};
}
...
function salvarProcedimento_(u,d){
  ...
  const tipoPreco = String(d.tipoPreco || 'A partir de').trim().toLowerCase() === 'por' ? 'Por' : 'A partir de';
  ...
  sh.getRange('B5').setFormula('=IF(OR(B3="",B4="",B3<=B4),"",ROUND((1-B4/B3)*100,0)&"% OFF")');
  sh.getRange('B6').setValue(tipoPreco);
  sh.getRange('B7').setValue(observacao);
  sh.getRange('B8').setValue(descricao);
  sh.getRange('B9').setValue(ativo);
  SpreadsheetApp.flush();
  limparCachePublico_();
...
function adicionarImagens_(u,d){
  ...
  fotos.forEach((f,index) => {
    ...
    sh.getRange(9 + slot, 2).setValue(url);
...
function excluirImagem_(u,d){
  ...
  const sh = planilha_().getSheetByName(cfg.sheet);
  const cell = sh.getRange(9 + slot, 2);
...
function lerProcedimento_(key, ssOpcional){
  ...
  return {
    ...
    priceMode: String(valorOuPadrao_(mapa,['tipo do preço','tipo do preco'],cfg.tipoPreco || 'A partir de')).trim() || 'A partir de',
    tipoPreco: String(valorOuPadrao_(mapa,['tipo do preço','tipo do preco'],cfg.tipoPreco || 'A partir de')).trim() || 'A partir de',
...
function configPorKey_(key){
  ...
  const cfg = procedimentosConfig_().find(x => x.key === k);
  if(!cfg) throw new Error('Procedimento inválido.');
  return cfg;
}

function garantirEstruturaPadroes_(ss){
  ss = ss || planilha_();
  sincronizarCadastroProcedimentos_(ss);
  CFG.PROCEDIMENTOS.forEach(cfg => {
    const sh = ss.getSheetByName(cfg.sheet);
    const mapa = sh ? lerMapaAba_(sh) : {};
    if(!sh || !Object.prototype.hasOwnProperty.call(mapa, normalizarChave_('Tipo do preço'))) configurarAbaProcedimento_(cfg);
  });
}

function garantirCadastroProcedimentos_(ss){
  ss = ss || planilha_();
  return criarAba_(ss, CFG.ABA_PROCEDIMENTOS, ['Key','Aba','Nome padrão','Ordem']);
}

function sincronizarCadastroProcedimentos_(ss){
  const sh = garantirCadastroProcedimentos_(ss);
  const atuais = objetos_(sh);
  CFG.PROCEDIMENTOS.forEach((cfg,index) => {
    if(!atuais.some(x => String(x.Key||'').trim().toLowerCase() === cfg.key)) sh.appendRow([cfg.key,cfg.sheet,cfg.nome,index+1]);
  });
  return sh;
}

function procedimentosConfig_(ssOpcional){
  const ss = ssOpcional || planilha_();
  const sh = ss.getSheetByName(CFG.ABA_PROCEDIMENTOS);
  if(!sh || sh.getLastRow() < 2) return CFG.PROCEDIMENTOS.slice();
  const defaults = {}; CFG.PROCEDIMENTOS.forEach(x => defaults[x.key] = x);
  const rows = objetos_(sh).map((r,i) => {
    const key = String(r.Key||'').trim().toLowerCase();
    const base = defaults[key] || {};
    return {
      key:key,
      sheet:String(r.Aba||base.sheet||r['Nome padrão']||'Procedimento').trim(),
      nome:String(r['Nome padrão']||base.nome||r.Aba||'Procedimento').trim(),
      precoPromocional:base.precoPromocional||'',
      observacaoPreco:base.observacaoPreco||'',
      tipoPreco:base.tipoPreco||'A partir de',
      descricao:base.descricao||'',
      ordem:Number(r.Ordem)||i+1
    };
  }).filter(x => x.key && x.sheet);
  rows.sort((a,b)=>a.ordem-b.ordem);
  return rows.length ? rows : CFG.PROCEDIMENTOS.slice();
}

function slug_(text){
  return String(text||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,48);
}
