...
function renderSwitcher(){
  const labels={micropigmentacao:"Micropigmentação",design:"Design",cilios:"Cílios"};
  $("#serviceSwitcher").innerHTML=Object.entries(state.procedures).map(([k,p])=>`<button type="button" class="service-btn ${k===state.currentKey?"active":""}" data-key="${esc(k)}"><strong>${esc(labels[k]||p.nome)}</strong><span>${p.active?"Visível no site":"Oculto"} • ${p.imagesDetailed?.length||0} foto(s)</span></button>`).join("");
...
function updatePreview(){
  ...
  $("#previewCurrent").textContent=p===null?"Consultar":money(p);
...
$("#procedureForm").addEventListener("submit",async e=>{
  ...
  try{
    const j=await api("salvarProcedimento",{key:state.currentKey,nome:f.elements.nome.value,precoOriginal:f.elements.precoOriginal.value,precoPromocional:f.elements.precoPromocional.value,observacaoPreco:f.elements.observacaoPreco.value,descricao:f.elements.descricao.value,ativo:f.elements.ativo.checked});
