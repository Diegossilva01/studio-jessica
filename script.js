...
function getFullImageUrl(src){
  ...
  if(!value) return "";
  // As imagens do Google Drive são exibidas em miniatura na grade; no modal pedimos uma versão maior.
  if(/drive\.google\.com\/thumbnail/i.test(value)){
    if(/[?&]sz=w\d+/i.test(value)) return value.replace(/([?&]sz=)w\d+/i, "$1w2400");
    return `${value}${value.includes("?") ? "&" : "?"}sz=w2400`;
  }
  return value;
}
...
function renderGallery(key, images){
  ...
  clean.forEach((src,index) => {
    ...
    item.setAttribute("aria-label", `Abrir foto ${index + 1} em tamanho maior`);
...
function initTabs(){
  document.addEventListener("click",event=>{
    ...
    // No celular, ao escolher um serviço, leva a cliente direto para as fotos dele.
    if(window.matchMedia("(max-width: 820px)").matches){
      requestAnimationFrame(()=>{
        setTimeout(()=>{
          const panel=[...document.querySelectorAll(".gallery-panel")].find(item=>item.dataset.panel===target);
          const gallery=panel?.querySelector(".photo-grid");
          if(!gallery)return;
...
function updatePhotoViewer(){
  ...
  if(!viewer || !image || !photoViewerItems.length) return;

  const current = photoViewerItems[photoViewerIndex];
  viewer.classList.add("is-loading");
  image.alt = current.alt || `Resultado ampliado ${photoViewerIndex + 1}`;
  image.onload = () => viewer.classList.remove("is-loading");
  image.onerror = () => viewer.classList.remove("is-loading");
  image.src = current.fullSrc || current.src;
  if(counter) counter.textContent = `${photoViewerIndex + 1} / ${photoViewerItems.length}`;

  const prev = viewer.querySelector(".photo-viewer-prev");
  const next = viewer.querySelector(".photo-viewer-next");
  const showNav = photoViewerItems.length > 1;
...
function openPhotoViewer(clickedImage){
  ...
  const gallery = clickedImage.closest(".photo-grid");
  if(!gallery) return;
  const images = [...gallery.querySelectorAll(".photo-item img")].filter(img => img.src);
  if(!images.length) return;

  photoViewerItems = images.map(img => ({
    src: img.currentSrc || img.src,
    fullSrc: img.dataset.fullSrc || getFullImageUrl(img.currentSrc || img.src),
    alt: img.alt
  }));
  photoViewerIndex = Math.max(0, images.indexOf(clickedImage));
  ...
  updatePhotoViewer();
  setTimeout(() => viewer.querySelector(".photo-viewer-close")?.focus(), 40);
}
...
function closePhotoViewer(){
  ...
  const image = document.getElementById("photoViewerImage");
  if(!viewer) return;
  viewer.classList.remove("open", "is-loading");
  viewer.setAttribute("aria-hidden", "true");
  document.body.classList.remove("photo-viewer-open");
  if(image){ image.removeAttribute("src"); image.onload = null; image.onerror = null; }
  photoViewerItems = [];
  photoViewerIndex = 0;
}
...
function movePhotoViewer(step){
  ...
  photoViewerIndex = (photoViewerIndex + step + photoViewerItems.length) % photoViewerItems.length;
  updatePhotoViewer();
}
...
function initPhotoViewer(){
  ...
  document.addEventListener("click", event => {
    ...
    if(item && item.closest(".photo-grid")){
      const img = item.querySelector("img");
      if(img) openPhotoViewer(img);
      return;
    }
    if(event.target.closest("[data-close-photo-viewer]")) closePhotoViewer();
  });

  document.addEventListener("keydown", event => {
    if(!viewer.classList.contains("open")){
      if((event.key === "Enter" || event.key === " ") && document.activeElement?.classList?.contains("photo-item")){
  ...
  viewer.querySelector(".photo-viewer-prev")?.addEventListener("click", event => { event.stopPropagation(); movePhotoViewer(-1); });
  viewer.querySelector(".photo-viewer-next")?.addEventListener("click", event => { event.stopPropagation(); movePhotoViewer(1); });
  ...
  viewer.addEventListener("touchend", event => {
    ...
    if(Math.abs(delta) > 55) movePhotoViewer(delta > 0 ? -1 : 1);
  }, {passive:true});
}
